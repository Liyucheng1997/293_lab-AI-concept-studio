...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    const { prompt, images = [], aspectRatio, imageSize, model, category, mode, title } = req.body || {};
    ...
    const item = {
      id, category: cat, mode: (typeof mode === "string" && /^[a-z]{1,20}$/.test(mode)) ? mode : null, file, url: `/files/${cat}/${file}`,
